...
const content = [
  ...
  {image: randomNumber, caption: 'Math.floor + Math.random', text: 'Math.floor - это метод, который округляет число до ближайшего меньшего целого числа. Math.random - это метод, который позволяет легко найти случайное число в определенном диапазоне', link: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Math/random', type: 'request', code: "function randomNumber(min, max) { return Math.floor(Math.random() * (max - min) + min) } console.log(randomNumber(0, 10)) console.log(randomNumber(12, 99))"},
  {image: arrayWithoutEmptyStrings, caption: 'Фильтрация пустых строк из массива', text: 'Это функция, которая находит все ложно подобное (пустые в это случаи) значения и не пропускает их', link: '#', type: 'lifehack', code: "const dirtyArray = ['JS', '', 'TS', '', '', 'PHP'] const clearArray = dirtyArray.filter(Boolean) console.log(clearArray)"},
  {image: arrayWithUniqueValues, caption: 'Фильтрация уникальных значений в массиве', text: 'Это функция, которая фильтрует уникальные значения через Set', link: '#', type: 'lifehack', code: "const dirtyArray = ['JS', 'TS', 'TS', 'PHP', 'PYTHON', 'TS'] let arrayWithUniqueValues = [... new Set(array)] console.log(arrayWithUniqueValues)"},
  {image: shuffledArray, caption: 'Перемешивание элементов в массиве', text: 'Это функция, которая перемешивает все значения в массиве через метод sort', link: '#', type: 'lifehack', code: "const array = ['JS', 'TS', 'PHP', 'PYTHON', 'Java'] const shuffledArray = array.sort( () => Math.random() - 0.5 ) console.log(shuffledArray)"},
  {image: falsyValues, caption: 'Ложно подобные значение', text: 'Ложно подобные (falsy) значение — значение, которое становится false в булевом контексте', link: 'https://developer.mozilla.org/ru/docs/Glossary/Falsy', type: 'lifehack', code: "const falsyValues = ['', 0, null, undefined, NaN, false]"},
  ...
  {image: findMaxValue, caption: 'Нахождение самого большого числа в массиве', text: 'Это функция, которая находит самое большое значение в массиве через цикличный перебор', link: '#', type: 'lifehack', code: "const numberArray = [1, 7, 26, 4, 82, 19] function findMaxNumber(array) { let maxNumber = array[0] for (const number of array) { if (maxNumber < number) maxNumber = number } return maxNumber } console.log(findMaxNumber(numberArray))"},
  {image: fizzBuzzTask, caption: 'Нахождение кратных чисел', text: 'Нахождение кратных чисел для 3 и 5', link: '#', type: 'lifehack', code: "const fizzBuzz = (limit) => { let out = '' for (let i = 0; i < limit; i++) { if (i % 3 === 0) out += ' fizz' else if (i % 5 === 0) out += ' buzz' else if (i % 3 === 0 && i % 5 === 0) out += ' fizzbuzz' else out += ' ' = i } return out } console.log(fizzBuzz(11)) console.log(fizzBuzz(22))"}, 
  {image: letterFinder, caption: 'Поиск буквы в слове', text: 'Поиск одной буквы в определенном слове', link: '#', type: 'lifehack', code: "function letterFinder(word, letter) { let firstCondition = typeof word === 'string' && word.length >= 2 let secondCondition = typeof word === 'string' && letter.length === 2 if (firstCondition && secondCondition) { for (let i = 0; i < word.length; i++) { if (word[i] == letter) return `Letter '${letter}' found at index ${i}` else return `Letter '${letter}' not found` } } else return 'Please pass correct arguments to the function' }"}, 
